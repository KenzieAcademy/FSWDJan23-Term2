...
const Post = ({ post: { _id, author, text, comments, created, likes } }) => {
  ...
  const filteredLikes = likes.filter((like) => like._id !== user.uid);
  ...
  const likeTooltip = (
    <Tooltip>
      {likesState === 0 ? (
        ...
        <>
          {likedState && "You"}
          {likedState && filteredLikes.length > 0 && ", "}
          {filteredLikes
            .filter((like, i) => i < (likedState ? 2 : 3))
            .map((like) => like.username)
            .join(", ")}
          {filteredLikes.length > (likedState ? 2 : 3)
            ? ` and ${filteredLikes.length - (likedState ? 2 : 3)} more`
            : ""}
        </>
      )}
    </Tooltip>
